...
define('WPADVQUIZ_VERSION', '1.0.4');
...
function wpAdvQuiz_myCred()
{
	$path = WPADVQUIZ_PATH . '/lib/' . 'plugin' . '/' . 'WpAdvQuiz_Plugin_myCredV1' . '.php';
	if(file_exists($path)) {
		include_once($path);
	}
}

add_action('plugins_loaded', 'wpAdvQuiz_myCred');