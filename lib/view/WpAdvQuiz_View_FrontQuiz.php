...
class WpAdvQuiz_View_FrontQuiz extends WpAdvQuiz_View_View
{
    ...
    public function showMaxQuestion()
    {
		...
		$allowed_html = wp_kses_allowed_html( 'post' );
		$result = wp_kses($result, $allowed_html);
    ...
    private function fetchCloze($answer_text)
    {
		...
		//$answerText = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $answerText);
		
		$allowed_html = wp_kses_allowed_html( 'post' );
		$answerText = wp_kses($answerText, $allowed_html);
    ...
    private function showStartQuizBox()
    {
        ...
        <div class="wpAdvQuiz_text">
            <p>
                <?php 
				$postBoxQuizDescription = $this->quiz->getText();
				//$postBoxQuizDescription = sanitize_text_field($postBoxQuizDescription);
				echo do_shortcode(apply_filters('comment_text', $postBoxQuizDescription)); ?>
            </p>
                ...
                <ul class="wpAdvQuiz_resultsList">
                    <?php foreach ($result['text'] as $resultText) { ?>
                        <li style="display: none;">
                            <div>
                                <?php 
								//$resultText = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $resultText);
								
								$allowed_html = wp_kses_allowed_html( 'post' );
								$resultText = wp_kses($resultText, $allowed_html);
                    ...
                    <li class="wpAdvQuiz_listItem" style="display: none;">
                        ...
                        <div class="wpAdvQuiz_question" style="margin: 10px 0 0 0;">
                            <div class="wpAdvQuiz_question_text">
                                ...
                                <?php echo do_shortcode(apply_filters('comment_text',wp_kses($question->getQuestion(),$allowed_html) )); ?>
                            </div>
                            ...
                            <ul class="wpAdvQuiz_questionList" data-question_id="<?php echo esc_attr($question->getId()); ?>"
                                ...
                                foreach ($answerArray as $v) {
									...
									//$answer_text = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $answer_text);
									
									$allowed_html = wp_kses_allowed_html( 'post' );
									$answer_text = wp_kses($answer_text, $allowed_html);					
                        ...
                        <?php if (!$this->quiz->isHideAnswerMessageBox()) { ?>
                            <div class="wpAdvQuiz_response" style="display: none;">
                                <div style="display: none;" class="wpAdvQuiz_correct">
									...
									//$_correctMsg = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $_correctMsg);

									$allowed_html = wp_kses_allowed_html( 'post' );
									$_correctMsg = wp_kses($_correctMsg, $allowed_html);	
                                ...
                                <div style="display: none;" class="wpAdvQuiz_incorrect">
									...
									//$_incorrectMsg = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $_incorrectMsg);	
									
									$allowed_html = wp_kses_allowed_html( 'post' );
									$_incorrectMsg = wp_kses($_incorrectMsg, $allowed_html);
                        ...
                        <?php if ($question->isTipEnabled()) { ?>
                            <div class="wpAdvQuiz_tipp" style="display: none; position: relative;">
                                <div>
									...
									$tipMsg = $question->getTipMsg();
									//$tipMsg = preg_replace('#<script(.*?)>(.*?)</script>#is', '', $tipMsg);
									
									$allowed_html = wp_kses_allowed_html( 'post' );
									$tipMsg = wp_kses($tipMsg, $allowed_html);

									do_shortcode(apply_filters('comment_text', $tipMsg)); ?>
                                </div>
                            </div>
                        <?php } ?>
						...
						<?php if (!$this->quiz->isBtnClearQuizHidden()) { ?>
						<input type="button" name="clear" value="<?php _e('Clear', 'wp-adv-quiz');  ?>"
                               class="wpAdvQuiz_button wpAdvQuiz_QuestionButton"
                               style="float: right !important; margin-right: 10px !important; display: block;
							   	width:<?php echo esc_attr($this->clear_btn_width); ?>px !Important; height:<?php echo esc_attr($this->clear_btn_height); ?>px !Important; background-color: <?php echo esc_attr($this->clear_btn_color); ?> !important">
						<?php } ?>
                     
				
						<?php  if ($this->quiz->isBtnViewPracticeShow()) { ?>
						<input type="button" name="practice" value="<?php _e('Practice', 'wp-adv-quiz');  ?>"
                               class="wpAdvQuiz_button wpAdvQuiz_QuestionButton" 
                               style="float: right !important; margin-right: 10px !important; display: block;
							   	width:<?php echo esc_attr($this->practice_btn_width); ?>px !Important; height:<?php echo esc_attr($this->practice_btn_height); ?>px !Important; background-color: <?php echo esc_attr($this->practice_btn_color); ?> !important">
						<?php } ?>
                        <div style="clear: both;"></div>
